...
var dmskrev = tryRequire( resolve( __dirname, './../lib/dmskrev.native.js' ) );
var opts = {
...
tape( 'the function has an arity of 5', opts, function test( t ) {
	t.strictEqual( dmskrev.length, 5, 'has expected arity' );
	t.end();
});
...
tape( 'the function returns a reference to the input array', opts, function test( t ) {
	...
	out = dmskrev( x.length, x, 1, mask, 1 );
...
tape( 'if provided an `N` parameter less than or equal to `0`, the function returns `x` unchanged', opts, function test( t ) {
	...
	dmskrev( 0, x, 1, mask, 1 );
	t.deepEqual( x, expected, 'returns expected value' );

	dmskrev( -4, x, 1, mask, 1 );
	t.deepEqual( x, expected, 'returns expected value' );
...
tape( 'the function supports specifying a mask stride', opts, function test( t ) {
	...
	expected = new Float64Array( [ 4.0, 2.0, 3.0, 1.0 ] );

	dmskrev( x.length, x, 1, mask, 2 );
	t.deepEqual( x, expected, 'returns expected value' );
